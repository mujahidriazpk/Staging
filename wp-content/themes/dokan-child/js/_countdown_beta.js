...
function time_is_widget() {
    ...
    this.init = function (a) {
        ...
        if (0 < q.length) {
            ...
            //b.setAttribute("src", "https://widget.time.is/js?" + encodeURIComponent(q.join("..")) + "&t=" + new Date().getTime());
            ...
            c.appendChild(b);
			//console.log("https://widget.time.is/js?" + encodeURIComponent(q.join("..")) + "&t=" + new Date().getTime());
        } else this.tick();
    };
            ...
            if (diff <= 0) {
                ...
                for (j in units) {
                    ...
                    if (p != -1) {
                        ...
                        if (c["units"] == "long") n= '<span class="countdown_amount '+d+'">'+n +'</span><br>' + d;
                        else if (c["units"] == "short") n = '<span class="countdown_amount '+c["units_abbr"][j]+'">'+n +'</span><br>'+ c["units_abbr"][j];
                        p2 = tm.indexOf("(");
                        p3 = tm.indexOf(")");
                        if (p2 < p && p < p3) {
                            tm2 = tm.substr(p2 + 1, p3 - p2 - 1).replace(units[j], n);
                            p3++;
                            if (diff < factor[j]) tm2 = '<span class="countdown_amount '+c["unit_names"][j]+'">0</span><br>' + c["unit_names"][j];
                        } else {
                            p2 = p;
                            p3 = p + units[j].length;
                            tm2 = n;
