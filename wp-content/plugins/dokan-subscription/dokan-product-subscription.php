...
class Dokan_Product_Subscription {
    ...
    function get_number_of_product_by_seller( $user_id ) {
        ...
        $query = "SELECT COUNT(*) FROM $wpdb->posts WHERE post_author = $user_id AND post_type = 'product' AND post_status IN ( 'publish', 'pending-review' )";
    ...
    function shortcode_handler() {
        ...
        <?php if ( Dokan_Product_Subscription::can_post_product() ): ?>
            <div class="seller_subs_info">
                <p>
                    <?php printf( __( 'Your are using <span>%s</span> package.', 'dps' ), $product->get_title() ); ?>
                </p>
                <p>
                    <?php printf( __( 'You can add <span>%s</span> product(s) for <span>%s</span> days.', 'dps' ), get_post_meta( $product->get_id(), '_no_of_product', true ), get_post_meta( $product->get_id(), '_pack_validity', true ) ) ?>
                </p>
                <p>
                ...
                ?>
                    ...
                    <div class="product_pack_item <?php echo ( $this->has_pack_validity_seller( get_the_ID() ) || $this->pack_renew_seller( get_the_ID() ) ) ? 'current_pack ' : ''; ?><?php echo ( ( get_post_meta( get_the_ID(), '_regular_price', true ) == '0' ) && $this->has_used_free_pack( get_current_user_id(), get_the_id() ) ) ? 'fp_already_taken' : ''; ?>">
                        ...
                        <div class="pack_content">
                            ...
                            printf( __( '<div class="pack_data_option"><strong>%d</strong> Products <br />', 'dps' ), $no_of_product );
                            ?>

                            <?php if ( $is_recurring && $recurring_interval >= 1 ) { ?>
                                <span class="dps-rec-period">
                                    <?php printf( __( 'In every %d %s(s)</div>', 'dps' ), $recurring_interval, $this->recurring_period( $recurring_period ) ); ?>
                                </span>
                            <?php } else {
                                $pack_validity = get_post_meta( get_the_id(), '_pack_validity', true );
                                printf( __( 'For<br /><strong>%s</strong> Days</div>', 'dps' ), $pack_validity );
                            } ?>
                        </div>
                        ...
                        <div class="buy_pack_button">
                            <?php if ( $this->has_pack_validity_seller( get_the_ID() ) ): ?>
                                ...
                                <a href="<?php echo get_permalink( get_the_ID() ); ?>" class="dokan-btn dokan-btn-theme buy_product_pack"><?php _e( 'Your Pack', 'dps' ); ?></a>

                            <?php elseif ( $this->pack_renew_seller( get_the_ID() ) ): ?>

                                <a href="<?php echo do_shortcode( '[add_to_cart_url id="' . get_the_ID() . '"]' ); ?>" class="dokan-btn dokan-btn-theme buy_product_pack"><?php _e( 'Renew', 'dps' ); ?></a>
                            ...
                            <?php else: ?>
                                ...
                                <?php elseif ( ! get_user_meta( get_current_user_id(), 'product_package_id', true ) ) : ?>
                                    ...
                                    <a href="<?php echo do_shortcode( '[add_to_cart_url id="' . get_the_ID() . '"]' ); ?>" class="dokan-btn dokan-btn-theme buy_product_pack"><?php _e( 'Buy Now', 'dps' ); ?></a>

                                <?php else:

                                    $btn_link = sprintf('<a href="%s" class="dokan-btn dokan-btn-theme buy_product_pack">%s</a>', get_permalink( get_the_ID() ), __( 'View Details', 'dps' ) ) ;
