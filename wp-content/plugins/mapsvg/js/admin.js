    ...
    methods = {
        ...
        setEventHandlersMainScreen : function(){
            ...
            $('#mapsvg-admin').on("click", "#mapsvg-btn-activate", function(e){
                ...
                $.post(ajaxurl, {purchase_code: code, action: "mapsvg_save_purchase_code", _wpnonce: MapSVG.nonce}, function(data){
            ...
            $("#svg_file_uploader_form").on('submit', function(form) {
                ...
                $.each(files, function(index, file) {
                    ...
                    $.ajax({
                        url: ajaxurl,
                        type: 'POST',
                        data: {action: 'mapsvg_upload', filename: filename, data: file.data, _wpnonce: MapSVG.nonce}
            ...
            $("#image_file_uploader_form").on('submit', function(form) {
                ...
                $.each(imgfiles, function(index, file) {
                    ...
                    $.ajax({
                        ...
                        window.location.href = '?page=mapsvg-config&map_id=new&package=uploads&path='+filenameFinal+'&noheader=true';
                    }).fail(function(response){
                        if(response.responseText){
                            response = JSON.parse(response.responseText);
                            if(response && response.data && response.data.error){
            ...
            $("#save-api-key").on('click', function() {
                ...
                if(key && key.length){
                    $.post(ajaxurl, {action: 'mapsvg_save_google_api_key', maps_api_key: key, geocoding_api_key: key2, _wpnonce: MapSVG.nonce}).done(function(data){
                        data = JSON.parse(data);
                        if(data.ok){
                            $('#myModal').modal('hide');
