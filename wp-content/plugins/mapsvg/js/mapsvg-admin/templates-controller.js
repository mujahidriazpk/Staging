(function($, window){
    ...
    MapSVGAdminTemplatesController.prototype.setEventHandlers = function() {
        var _this = this;
        this.toolbarView.find('select').mselect2().on("change",function () {
            _this.setEditor();
        });
    };
    ...
    MapSVGAdminTemplatesController.prototype.setTextareaValue = function(codemirror, changeobj){
        ...
        if(textarea.attr('name') === 'templates[labelMarker]' && this.mapsvg.getData().options.labelsMarkers.on){
            this.mapsvg.setLabelsMarkers();
        }
        if(textarea.attr('name') === 'templates[labelRegion]' && this.mapsvg.getData().options.labelsRegions.on){
