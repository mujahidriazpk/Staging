    ...
    MapSVGAdminDrawController.prototype.setEventHandlers = function(){
        ...
        if(_this.readOnly){
            $('body').on('click.mapsvg-save-copy', '#mapsvg-save-svg-copy',function(){
                ...
                $.post(ajaxurl, {action: 'mapsvg_svg_copy', filepath: _this.filepath, _wpnonce: MapSVG.nonce})
                    .done(function(data){
    ...
    MapSVGAdminDrawController.prototype.editingShapeUnset = function() {
        ...
        if(_this.editingShape && _this.editingShape.obj){
            ...
            if(_this.editingShape.obj.getAttribute('class').indexOf('active') != -1 ){
                _this.editingShape.obj.setAttribute('class', _this.editingShape.obj.getAttribute('class').replace(/active/g,''));
            }
            ...
            .done(function(xmlData){
                ...
                newSVG.find('path, polygon, circle, ellipse, rect').each(function(index){
                    ...
                    if($(this).attr('class')){
                        ...
                        $(this).attr('class', $(this).attr('class').replace(/mapsvg-custom-shape/g,''));
                        $(this).attr('class', $(this).attr('class').replace(/mapsvg-region/g,''));
                        $(this).attr('class', $(this).attr('class').replace(/mapsvg-region-hover/g,''));
                        $(this).attr('class', $(this).attr('class').replace(/mapsvg-disabled/g,''));
                    }
                });
                ...
                $.post(ajaxurl, {action: 'mapsvg_save_svg', _wpnonce: MapSVG.nonce, map_id: _this.mapsvg.id, body: newSVG[0].outerHTML.replace(/\sgeoViewBox/ig,' mapsvg:geoViewBox'), filepath: _this.filepath})
                    .done(function(data){
                        newSVG = null;
