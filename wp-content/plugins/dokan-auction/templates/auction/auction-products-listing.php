        ...
        <?php do_action( 'dokan_before_listing_auction_product' ); ?>
            ...
            <article class="dokan-product-listing-area">
                ...
                <table class="table table-striped product-listing-table">
                    <thead>
                        <tr>
                            <th><?php _e( 'Image', 'dokan-auction' ); ?></th>
                            <th><?php _e( 'Name', 'dokan-auction' ); ?></th>
                            <th><?php _e( 'Status', 'dokan-auction' ); ?></th>
                            <th><?php _e( 'SKU', 'dokan-auction' ); ?></th>
                            <th><?php _e( 'Stock', 'dokan-auction' ); ?></th>
                            <th><?php _e( 'Price', 'dokan-auction' ); ?></th>
                            <th><?php _e( 'Type', 'dokan-auction' ); ?></th>
                            <th><?php _e( 'Views', 'dokan-auction' ); ?></th>
                            <th><?php _e( 'Date', 'dokan-auction' ); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                                ...
                                <tr<?php echo $tr_class; ?>>
                                    ...
                                    <td>
                                        ...
                                        if ( $product->is_in_stock() ) {
                                            echo '<mark class="instock">' . __( 'In stock', 'dokan-auction' ) . '</mark>';
                                        } else {
                                            echo '<mark class="outofstock">' . __( 'Out of stock', 'dokan-auction' ) . '</mark>';
                                        }
                                    ...
                                    <td class="post-date">
                                        ...
                                        if ( '0000-00-00 00:00:00' == $post->post_date ) {
                                            $t_time = $h_time = __( 'Unpublished', 'dokan-auction' );
                                            $time_diff = 0;
                                        } else {
                                            $t_time = get_the_time( __( 'Y/m/d g:i:s A', 'dokan-auction' ) );
                                            ...
                                            if ( $time_diff > 0 && $time_diff < 24 * 60 * 60 ) {
                                                $h_time = sprintf( __( '%s ago', 'dokan-auction' ), human_time_diff( $time ) );
                                            } else {
                                                $h_time = mysql2date( __( 'Y/m/d', 'dokan-auction' ), $m_time );
                                            }
                                        }
                                        ...
                                        if ( 'publish' == $post->post_status ) {
                                            _e( 'Published', 'dokan-auction' );
                                        } elseif ( 'future' == $post->post_status ) {
                                            if ( $time_diff > 0 ) {
                                                echo '<strong class="attention">' . __( 'Missed schedule', 'dokan-auction' ) . '</strong>';
                                            } else {
                                                _e( 'Scheduled', 'dokan-auction' );
                                            }
                                        } else {
                                            _e( 'Last Modified', 'dokan-auction' );
                                        }
                                        ?>
                                    </td>
                                </tr>
