<?php
/*
Plugin Name: Dokan - Auction Integration
...
class Dokan_Auction {
    ...
    public function __construct() {
        ...
        $this->depends_on['dokan'] = array(
            'name'   => 'WeDevs_Dokan',
            'notice' => sprintf( __( '<b>Dokan - Auction Integration </b> requires %sDokan plugin%s to be installed & activated!' , 'dokan-auction' ), '<a target="_blank" href="https://wedevs.com/products/plugins/dokan/">', '</a>' ),
        );

        $this->depends_on['WooCommerce_simple_auction'] = array(
            'name'   => 'WooCommerce_simple_auction',
            'notice' => sprintf( __( '<b>Dokan - Auction Integration </b> requires %sWooCommerce Simple Auctions plugin%s to be installed & activated!' , 'dokan-auction' ), '<a target="_blank" href="https://codecanyon.net/item/woocommerce-simple-auctions-wordpress-auctions/6811382">', '</a>' ),
        );
        ...
        add_action( 'admin_init', array( $this, 'load_updater' ), 99 );
    ...
    **/
    ...
    public function localization_setup() {
        load_plugin_textdomain( 'dokan-auction', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
    }
    ...
    public function load_updater() {
        if ( is_admin() ) {
            require_once dirname(__FILE__) . '/includes/libs/update.php';
            new Dokan_Auction_Update( 'dokan-auction' );
        }
    }
    ...
    **/
    ...
    function add_admin_user_options( $user ) {
        ...
        ?>
        <tr>
            <th><?php _e( 'Auction', 'dokan-auction' ); ?></th>
            <td>
                <label for="dokan_disable_auction">
                    ...
                    <?php _e( 'Disable Auction', 'dokan-auction' ); ?>
                </label>

                <p class="description"><?php _e( 'Disable auction capability for this vendor', 'dokan-auction' ) ?></p>
            </td>
        </tr>
        <?php
    }
    ...
    function add_auction_dokan_settings( $settings_fields ) {
        $settings_fields['dokan_selling']['new_seller_enable_auction'] = array(
            ...
            'label'   => __( 'New vendor Enable Auction', 'dokan-auction' ),
            'desc'    => __( 'Make auction status enable for new registred vendor', 'dokan-auction' ),
            'type'    => 'checkbox',
            'default' => 'on'
        );
    ...
    function add_auction_dashboad_menu( $urls ) {
        ...
        if ( dokan_is_seller_enabled( get_current_user_id() ) && !dokan_is_seller_auction_disabled( get_current_user_id() ) ) {
            $urls['auction'] = array(
                'title' => __( 'Auction', 'dokan-auction' ),
    ...
    **/
    ...
    function load_dokan_auction_template( $query_vars ) {
        if ( ! current_user_can( 'dokan_view_auction_menu' ) ) {
            dokan_get_template_part('global/dokan-error', '', array( 'deleted' => false, 'message' => __( 'You have no permission to view this auction page', 'dokan-auction' ) ) );
            return;
        }
...
function dokan_load_acution_plugin() {
    $dokan_auction = Dokan_Auction::init();
}

add_action( 'plugins_loaded', 'dokan_load_acution_plugin', 9 );
register_activation_hook( __FILE__, array( 'Dokan_Auction', 'activate' ) );
