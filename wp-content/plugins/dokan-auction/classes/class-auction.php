    ...
    function auction_handle_all_submit() {
        ...
        if ( isset( $_POST['add_auction_product'] ) && wp_verify_nonce( $_POST['dokan_add_new_auction_product_nonce'], 'dokan_add_new_auction_product' ) ) {
            ...
            if ( !self::$errors ) {
                ...
                if ( $product_id ) {
					...
					$auction_no = $US_state[$billing_state].$postcode.'-'.date('Y-md',strtotime($_POST['_auction_dates_from']))."-".$fourRandomDigit."-".str_pad($product_id,4,'0', STR_PAD_LEFT);
		...
		//Mujahid Code
           ...
           if ( !self::$errors ) {
			...
			$featured_flag = 'No';
			$gallery = array();
			//Reset auction images
			delete_post_thumbnail($product_id);
			update_post_meta( $product_id, '_product_image_gallery','');
			update_post_meta( $product_id, 'feat_image_id_periapical','');
			update_post_meta( $product_id, 'feat_image_id_panorex','');
			update_post_meta( $product_id, 'feat_image_id_bitewing','');
			update_post_meta( $product_id, 'feat_image_id_full_mouth','');
			update_post_meta( $product_id, 'feat_image_id_periapical_2','');
			update_post_meta( $product_id, 'feat_image_id_sample1','');
			update_post_meta( $product_id, 'feat_image_id_sample2','');
			update_post_meta( $product_id, 'feat_image_id_sample3','');
			update_post_meta( $product_id, 'feat_image_id_sample4','');
